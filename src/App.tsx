...
const IntroScreen: React.FC<{ onStart: () => void }> = ({ onStart }) => {
    ...
    return (
        <div 
            className={`fixed inset-0 z-[200] intro-bg flex flex-col items-center justify-center font-rajdhani overflow-hidden ${exiting ? 'intro-exit' : ''}`}
        ...
        >
            ...
            <div 
                ref={cardRef}
                className="glass-card-intro relative z-10 p-12 md:p-16 rounded-[2rem] max-w-4xl w-[90%] text-center flex flex-col items-center border-t border-white/20"
            >
                {/* Header Decoration */}
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1/3 h-1 bg-gradient-to-r from-transparent via-cyan-400 to-transparent shadow-[0_0_15px_#00f3ff]"></div>
                
                {/* Title Block */}
                <div className="mb-8 animate-[fadeIn_1s_ease-out]">
                    <h1 className="text-5xl md:text-7xl font-bold tracking-tighter mb-2 title-gradient font-[Orbitron] uppercase leading-tight">
                        Flood-Aware <br/>
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500 text-glow-cyan text-4xl md:text-6xl">Real-Time Navigation</span>
                    </h1>
                    <div className="h-[1px] w-24 bg-cyan-500/50 mx-auto mt-6 mb-6"></div>
                    <p className="text-lg md:text-xl text-blue-200 tracking-[0.3em] font-light uppercase">
                        Smart Navigation With Flood Hazard Detection
                    </p>
                </div>

                {/* Enter Button */}
                <div className="mt-8 relative group animate-[fadeIn_1s_ease-out_0.5s_forwards] opacity-0">
                    <button 
                        onClick={handleEnter}
                        className="magical-btn px-12 py-5 rounded-full flex items-center gap-4 text-white font-bold text-xl tracking-widest uppercase"
                    >
                        <div className="btn-pulse"></div>
                        <span className="relative z-10 group-hover:text-cyan-100 transition-colors">Enter Project</span>
                        <ArrowRight className="w-6 h-6 relative z-10 group-hover:translate-x-2 transition-transform text-cyan-400" />
                    </button>
                </div>

                {/* Decorative Tech Elements */}
                <div className="absolute bottom-6 left-8 flex items-center gap-2 opacity-50 text-cyan-200 text-xs font-mono tracking-widest">
                    <Activity className="w-4 h-4" /> SYSTEM READY
                </div>
                <div className="absolute bottom-6 right-8 flex items-center gap-2 opacity-50 text-cyan-200 text-xs font-mono tracking-widest">
                    v2.5.0 <Cpu className="w-4 h-4" />
                </div>
            </div>

            {/* Footer */}
            <div className="absolute bottom-8 text-center opacity-0 animate-[fadeIn_1s_ease-out_1s_forwards]">
                <p className="text-slate-400 text-xs tracking-[0.5em] font-light">
                    POWERED BY AI • SECURE GEOLOCATION • REAL-TIME DATA
                </p>
            </div>
        </div>
    );
...
const App: React.FC = () => {
  // --- App State ---
  const [hasStarted, setHasStarted] = useState(false); // Controls transition from Intro to Main App
  ...
  const handleSystemStart = () => {
      // Transition logic handled inside IntroScreen (fade out), then this is called
      setHasStarted(true);
      // Boot Sequence
      setTimeout(() => setInitStep(1), 500);
      setTimeout(() => setInitStep(2), 1200);
      setTimeout(() => setIsSystemReady(true), 1800);
      speak("System initialized. Welcome to HydroNav.");
      
      // Attempt generic locate on start
      if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(
              ...
              (err) => console.warn("Initial fix failed", err),
              { timeout: 5000 }
          );
      }
  };
  ...
  useEffect(() => {
    // DO NOT START GPS UNTIL INTRO IS PASSED
  ...
  }, [isSimulating, navStatus.isNavigating, destination, hasStarted]); 
  ...
  return (
    <div className="relative h-screen w-screen bg-[#020617] text-white overflow-hidden font-rajdhani flex flex-col md:block">
      ...
      {/* --- INTRO PAGE OVERLAY --- */}
      {!hasStarted && (
          <IntroScreen onStart={handleSystemStart} />
      )}

      {/* SYSTEM INIT / REBOOT OVERLAY */}
      {hasStarted && (!isSystemReady || isResetting) && (
          <div className="absolute inset-0 z-[100] bg-[#020617] flex flex-col items-center justify-center">
              ...
              <div className="flex flex-col items-center space-y-1">
                  {isResetting ? (
                      ...
                      <>
                        <div className={`text-xs uppercase tracking-widest transition-colors duration-300 ${initStep >= 0 ? 'text-cyan-400' : 'text-slate-700'}`}>[ OK ] Connecting GPS Satellites...</div>
                        <div className={`text-xs uppercase tracking-widest transition-colors duration-300 ${initStep >= 1 ? 'text-cyan-400' : 'text-slate-700'}`}>[ OK ] Loading Flood Topography...</div>
                        <div className={`text-xs uppercase tracking-widest transition-colors duration-300 ${initStep >= 2 ? 'text-cyan-400' : 'text-slate-700'}`}>[ OK ] System Calibrated</div>
                      </>
                  )}
              </div>
          </div>
      ...
      {!navStatus.isNavigating && hasStarted && (
        <div className="absolute top-0 left-0 w-full p-2 md:p-4 z-40 pointer-events-none flex justify-between items-start">
            <div className="glass-panel px-3 py-2 md:px-4 md:py-2 rounded-full flex items-center space-x-2 md:space-x-3 pointer-events-auto shadow-lg animate-in slide-in-from-top-4 duration-700">
                ...
                <div>
                    <h1 className="text-lg md:text-xl font-bold tracking-wider leading-none text-white">
                        HYDRONAV <span className="hidden md:inline text-cyan-400 text-xs align-top font-normal tracking-widest">OS v2.4</span>
                    </h1>
                </div>
            </div>
            {/* Status Pills & Controls */}
            <div className="flex items-center gap-3 pointer-events-auto animate-in slide-in-from-top-4 duration-700 delay-100">
                <button 
                    onClick={handleReset}
      ...
      {!navStatus.isNavigating && hasStarted && (
          ...
          `}>
             <div className="glass-panel rounded-t-2xl md:rounded-2xl p-4 shadow-2xl relative overflow-visible bg-[#020617]/90 md:bg-var(--glass-bg) animate-in slide-in-from-left-4 duration-700">
